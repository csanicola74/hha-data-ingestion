...
apiDataset = requests.get('https://data.cms.gov/data-api/v1/dataset/0e312aa0-3508-414c-a874-35e42122f123/data')
...
client = bigquery.Client.from_service_account_json('/Users/carolinesanicola/Documents/GitHub/hha-data-ingestion/caroline-507-c98b1afca13c.json')
# 5. create 'query_job' variable to use key to access public database and limit the table to the first 100 rows
query_job = client.query("SELECT * FROM `bigquery-public-data.google_analytics_sample.ga_sessions_*` LIMIT 100")
...
client = bigquery.Client.from_service_account_json('/Users/carolinesanicola/Documents/GitHub/hha-data-ingestion/caroline-507-c98b1afca13c.json')
query_job = client.query("SELECT * FROM `bigquery-public-data.hacker_news.comments` LIMIT 100")
results = query_job.result()
